...
| decode-dbo / crosslayer | representative ITL sample，即一个代表性 decode step 的 `total_time_ms` | serial TPOT，即 `decode_step_ms`；缺失时才用 TPOT fallback | `serial_TPOT / DBO_representative_ITL` |
...
### TPOT 与 `total_time_ms / max_new_tokens`

TPOT（Time Per Output Token）通常指 decode 阶段每输出一个 token 的平均耗时。如果 `total_time_ms` 覆盖的是纯 decode 段，那么：

```text
TPOT = decode_total_time_ms / output_tokens
```

但当前很多 serial cache 的 `total_time_ms` 是 full generation latency，包含 prefill / TTFT-path。这时：

```text
total_time_ms / max_new_tokens
```

是“把 TTFT 均摊进每个输出 token 的 per-output-token 近似值”，可作为 decode 的 TPOT fallback，但必须标注；它不能用于 TTFT speedup。
...
- TPOT 缺 `decode_step_ms` 时允许 fallback 到 `total_time_ms / max_new_tokens`，并在 report 中标注 fallback 来源。
...
|---|---|---:|---:|
| decode-dbo | native serial TPOT (`decode_step_ms`) | 15 | 0.340-0.734 |
| decode-dbo | TPOT fallback (`total_time_ms/tokens`) | 9 | 1.080-1.327 |
| decode-dbo-crosslayer | native serial TPOT (`decode_step_ms`) | 15 | 0.337-0.683 |
| decode-dbo-crosslayer | TPOT fallback (`total_time_ms/tokens`) | 9 | 1.090-1.427 |
| prefill-dbo | native serial TTFT-path (`prefill_ms`) | 15 | 1.048-1.439 |

结论：

- GPU decode/crosslayer 图上的可补 N/A 已通过 TPOT fallback 重新生成。
- GPU prefill 当前已有 15 个 `prefill_ms`，可以给可信 TTFT-path speedup；GPU serial cache 共有 24 个，其中 9 个大 batch 配置仍缺拆分字段。
- GPU decode 不是“全负收益”：小/中 batch 的 native TPOT baseline 多数 <1，大 batch fallback 配置多数 >1；但 fallback 数值可能受 serial full-generation 均摊 TTFT 影响，应优先补 `decode_step_ms`。

### NPU `results_npu/`

| 子模式 | baseline 状态 | 数量 | speedup 范围 |
|---|---|---:|---:|
| decode-dbo | TPOT fallback (`total_time_ms/tokens`) | 24 | 1.123-4.049 |
| decode-dbo | serial cache missing | 21 | N/A |
| decode-dbo-crosslayer | TPOT fallback (`total_time_ms/tokens`) | 24 | 1.209-4.063 |
| decode-dbo-crosslayer | serial cache missing | 21 | N/A |
| prefill-dbo | serial TTFT-path (`prefill_ms`) missing | 18 | N/A |

结论：

- NPU decode 在已有 serial cache 的 48 个配置上可以补 TPOT speedup，但全是 fallback，不是 native `decode_step_ms`。
- NPU `seq=1024/2048` 和部分 `b512` decode 缺 serial cache，不能凭空补 speedup。
- NPU prefill 现有 18 个配置都缺 `prefill_ms`，旧 report 中的 TTFT speedup 不可信，应改为 N/A，直到补跑 serial TTFT-path baseline。
...
| `decode_step_ms` | serial TPOT baseline | 可用于 TPOT speedup |
| `total_time_ms` | mode-dependent total；serial generation cache 中通常是 full generation latency | 不能单独使用；decode 缺 `decode_step_ms` 时可配合 `max_new_tokens` 做 TPOT fallback |
| `max_new_tokens` | 输出 token 数 | 只用于 TPOT fallback |

严格 speedup 应优先使用 `prefill_ms` 和 `decode_step_ms`。`scripts/capture_serial_prefill.sh` 会对同一配置额外跑 serial prefill-only，把 `prefill_ms` 写回 cache，并推导 `decode_step_ms`，用于补全拆分字段。
...
`total_time_ms / max_new_tokens` 是可接受的 TPOT 兜底；如果 `total_time_ms` 是纯 decode 段，它就是 TPOT。但当前很多 serial cache 的 total 是 full generation latency，会把 TTFT-path 均摊到每个 token 上，fallback 会偏大，从而高估 DBO TPOT speedup。

所以 NPU decode 的 1.1-4.1x 可以作为“现有 cache 下的近似 TPOT 对比”，但严格结论需要补 `decode_step_ms` 或重新跑只计 decode/TPOT 的 serial baseline。

### 4.3 representative ITL sample 不是 TPOT 平均值

当前 DBO decode 图中的 DBO 时间来自一个 representative ITL sample。实现上，`DecodeDBOScheduler` 初始化 `_timing_step = 1`，每次 `forward_decode_dbo()` 后递增 `_current_step`；只有 `_current_step == 1` 时创建 `TimingTracker`。也就是说，图里记录的是第 2 个 DBO decode call，第 0 个 step 被跳过以规避 lazy init、allocator、cache 和通信冷启动。

这类 representative ITL sample 适合画 pipeline Gantt、看每层 overlap/bubble，但它不是多步平均 TPOT。服务指标对比仍应优先使用 serial `decode_step_ms` 与 DBO representative step保持一致时的近似对比；若要严格 TPOT，应记录并平均多个稳定 decode step。
...
2. **GPU TPOT 是混合结果。** 小/中 batch native TPOT baseline 多数 <1，大 batch fallback 配置多数 >1；不能简单说 GPU decode 全负。
3. **NPU TPOT 在已有 cache 的配置上显示明显正收益。** 但 48 个可算 speedup 的配置全部依赖 fallback，且缺失更大 seq/batch 的 serial baseline。
...
- GPU TTFT-path 正收益可信；
- GPU TPOT 在大 batch 可能转正，但需要 native serial `decode_step_ms` 补强；
- NPU TPOT 近似对比显示正收益，尤其大 batch，但需要补 serial `decode_step_ms`；
