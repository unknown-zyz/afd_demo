...
Pipeline Gantt 图展示 decode step 1 的 layer / micro-batch 事件，但所有报告和
audit speedup 都使用上表中的准确 TTFT/TPOT 字段。
...
4. 两个结果目录的 baseline 完整性已清理干净：当前报告没有依赖单个 decode step
   timing 或 `total_time_ms/tokens` fallback。
