...
### 3.4.4 为什么 MB1 的 F2A 比 MB0 长很多（4-lane round-4 解读）

EP7 `b16/s512/t20` L3 实测：

| 事件 | MB0 (ms) | MB1 (ms) |
|---|---|---|
| FFN.send_transfer.start (combine 即将完毕) | 34.13 | 34.28 |
| ATT.recv_wait.start (ATT 端 post irecv 时刻) | 29.29 | **35.59** |
| ATT.recv_wait.end (ATT 真正拿到数据) | 34.33 | 35.62 |
| **F2A bar 长度** = send_start → recv_wait_end | **0.20** | **1.34** |

**MB1 F2A 长不是网络慢、不是 combine 慢，而是 ATT 端 recv 串行调度 + 当前层 attn 已退场**：

1. ATT MB0 在 attn 一算完（29.29）就 pre-post 了 MB0 irecv，所以 MB0 数据 ~34.18 一进 NCCL 缓冲就被 wait 命中，bar 短。
2. MB1 数据 ~34.32 也已经到达 ATT 端 NCCL 缓冲，但 ATT 是顺序的：先 wait MB0（29.29 → 34.33 占住 5 ms），完成才 post MB1 irecv（35.59）。这段时间 MB1 数据"无人消费"，仅在缓冲里干等 ~1.27 ms。
3. MB1 自身 recv_wait 实际只等了 26 µs（拿到就走），但 bar 头部锚在 FFN.send_start=34.28、尾部锚在 ATT.recv_wait_end=35.62 ⇒ 1.34 ms。

**这是 DBO pipeline 中真实的可优化点，不是绘图 bug**：

- 最直接的修法：在 ATT 一进 recv 阶段就并发 post 两个 MB 的 irecv，避免"先 wait MB0 才 post MB1"；
- 或者使用 `--crosslayer`，让下一层 ATT 提前 post irecv，重叠掉本层 F2A 的尾巴。

Round-4 4-lane 图保留这个长度（不替换为"网络到达时刻"），是为了**让 bar 长度真实反映"通信链路 + 接收侧排队"总时长**，作为 pipeline 优化的指示器。

### 3.4.5 hidden in-flight 这段时间在做什么，新图为什么不再单画

- `ep_overlap_hidden` event 是"reduce 已发出但下一个 MB 的 compute 已经开始"那段；这段时间 reduce 通信仍在跑，**combine 正在进行**，但占的是 NPU 的通信通道而不是计算单元。
- Round-3 staged 图把它画成 xx 斜线 overlay，用来说明"长 wall-span ≠ wait"。教学价值已达成。
- Round-4 4-lane 视角更聚焦"compute vs comm 资源利用率"：combine 整段进入 **F2A 通信泳道**（与 NPU send/recv 同泳道），hidden 段自然变成"F2A 通信窗口的一部分"，不必再单独画。原始 `ep_overlap_hidden` 仍保留在 timing JSON 中，分析时可读取。

### 3.5 最终判断（Round-4 4-lane 视角）

新版默认绘图为 4 泳道（`--ffn-view fourlane`）：**Attention / A2F / FFN / F2A**。语义：

- A 行 = `attn_compute`（ATT GPU 计算）。
- A2F 行 bar = `[ATT.send.start, FFN.ep_local_experts.start]`（传输开始 → FFN 真正进入 GEMM）；包含 send + recv_wait + router + dispatch。
- FFN 行 bar = `ep_local_experts` 主体（fallback：`moe_experts` / `ffn_compute`）。
- F2A 行 bar = `[FFN.ep_reduce.start, ATT.recv_wait.end]`（combine 开始 → ATT 真正拿到数据）；包含 combine + reduce_wait + send + ATT recv_wait（含串行排队）。

| 现象 | 是否合理 | 解释 |
|---|---|---|
| A2F / F2A 的 MB0 与 MB1 bar 长度不一致 | 合理 | bar = 传输开始 → 接收方 wait 完成；接收方 post irecv 越晚，bar 头-尾错位越大（见 3.4.4 MB1 F2A） |
| L3 A2F 比 L1 长 | 真实层间空泡 | 非 crosslayer 下 FFN 没机会提前 post irecv，A2F bar 尾部被 FFN-端排队拉长 |
| MB1 F2A 长达 1.34 ms 而 MB0 仅 0.2 ms | **真实可优化瓶颈，不是绘图 bug** | ATT 端串行 recv：先 wait MB0 才 post MB1 irecv；MB1 数据已就位却无人消费 → 优化方向：并发 post 两 MB 的 irecv，或开 `--crosslayer` 提前 post 下一层 |
| FFN MB0 与 MB1 bar 大面积重叠 | 不合理 | 单 NPU compute stream 串行；若新 4-lane 图仍看到，说明 timing 数据有重建误差或 fallback 到 ffn_compute |
| MB0 F2A 与 MB1 FFN 重叠 | 合理且期望 | DBO 核心收益：FFN 计算通道与 F2A 通信通道并行 |
| 4-lane 图未画 hidden 斜线 | 合理 | combine 整段在 F2A 通信泳道，hidden 已隐含在 F2A bar 内（见 3.4.5） |
