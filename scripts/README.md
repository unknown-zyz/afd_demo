...
| `run_npu_coordinator_single_host.sh` | Host1 单机 1A1F coordinator smoke：先起 gRPC coordinator，再走真实 `src.main` decode/prefill 路径。当前仅支持 1A1F、无 FFN EP。 |
...
- `comm_timing_mode=enqueue` 时，A2F/F2A bars 是非阻塞 `isend()` 返回开销，不是
  ...
  用于观察通信是否被计算掩盖。completion span 包含真实数据搬运，但也可能包含
  backend 排队、接收端 readiness、通信流调度和完成通知，因此不是纯硬件链路传输时间。
...
用于回答 “DBO pipeline 图里的 completion span 与独立通信耗时是否一致”。
...
输出里的 `sender_completion_ms` 是 `isend()` 发起到 `Work.wait()` 完成的跨度。
